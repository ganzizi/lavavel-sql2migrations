...
foreach ($table_names as $table_name){
    ...
    if($result = $mysqli->query($query)){
        while($row = $result->fetch_assoc()){
            ...
            $field_type_name_old = $field_type_name = $field_type_settings_split[0];
            ...
            if($extra == 'auto_increment'){
                $field_type_name = array_key_exists($field_type_name_old, $field_type_increment_mappings) ? $field_type_increment_mappings[$field_type_name_old] : $field_type_name;
            }
            ...
            //     $appends []= '->unsigned()';
            // }
            if('unsigned' === $field_type_setting){
                $appends []= '->unsigned()';
            }
            if(!is_null($default)){
                if($default == 'CURRENT_TIMESTAMP'){
                    ...
                    if ($field_type_name === 'longText') {
                        $appends []= sprintf("->default('')", $default);
                    }else{
    ...
    while($row = $results->fetch_array()){
        if ($row[2] != 'PRIMARY') {
            ...
            // $table_schema_codes[] = '    $table->primary(["' . $row[4] .'"]);';
        }
    }
    if (!empty($indexes)) {
    ...
    $classname = sprintf("Create%sTable", str_replace(' ', '', str_replace('_', ' ', ucwords($echo_table_name))));
...
class $classname extends Migration
{
    ...
    public function up()
    {
        Schema::create('$echo_table_name', function (Blueprint \$table) {
        $table_schema_codes
        });
    }
    ...
    {
        Schema::dropIfExists('$echo_table_name');
    }
}

";
    $output_file_name = sprintf("%s/%s_create_%s_table.php", $folder, $datetime_prefix, $echo_table_name);
    // echo $code . "\n";
